...
> **Restored on 2026-09-02.** This file was deleted in `35a280a` ("Drop the
> decision prose and the UI contract test") together with `WAVE.md`,
> `DESK_PASS_FINDINGS.md` and `test/ui-contract.test.ts`. The test was the
> friction that commit removed — 283 assertions pinning class names — and it
> stays deleted; `test/material.test.ts` is its better successor. The prose was
> collateral, and the session that followed was written against it: every
> non-obvious call in this repo is recorded here as the bug it prevents, and a
> repo whose comments say "see the decision log" with no log is one whose next
> reader re-derives the same bugs. The two scratch files stay gone — they were
> closed-out working notes, not decisions. Entries 1–53 are as they were at
> `8b03ecd`; 54 onward are this session's.
...
## 54. The Slack scope is a table, and `#truto` is back

**Supersedes #49.** #49 narrowed `DESK_CHANNELS` in `env.ts` to the channels the
operator listed, and it was right to narrow. What it did on the way was drop
`#truto` — the team's own channel — because the list it was given did not name
it, and it recorded that as a decision. The prompt that opened this session named
`#truto` explicitly as the channel that should be on the desk ("if the operator
says *Pluto* channel, this is almost certainly what is meant"), so the two
instructions disagreed and the later one wins. But the real lesson is the
mechanism: a hand-edited array was edited twice inside one week, and the second
edit changed what reaches the desk in a diff about something else.

`slack_channels` replaces it (migration 15, `slackScope.ts`, a Settings section).
One row per channel, three independent facts:

| fact | values | what it decides |
|---|---|---|
| `mode` | `off` · `mentions` · `all` | nothing · only `<@me>` hits (search, `in:#name`) · the channel read as history every poll |
| `label` | team · customer · partner · alert · crisp | what the row *is* to a person; `alert`/`crisp` only legal when a `family` is set |
| `family` | sentry · datadog · grafana · crisp | which parser reads a history channel; discovered, never client-settable |

Seeded from a live read of the workspace on 2026-09-02 (`slack_list_user_channels`,
39 channels the identity can see):

* `#truto` `C04D9HKDWAV` — team, **mentions**. Six top-level posts a day, most of
  them aimed at other people; reading it wholesale would have put eighty rows on
  the desk in a fortnight, measured on the channel's own history.
* Fourteen `*-truto` channels plus `#sprinto` — customer, **all**. They are Slack
  Connect: `#stax-truto` has six members with `@stax.ai` addresses. A customer's
  question does not reliably name him, so a search cannot see it and a channel read
  can.
* `#clonepartner` `C09BRBLNXNH` — partner, **all**; `#framer-clonepartner`
  `C06UP5J326B` — partner, **off**. Two channels match "clone partner"; the first
  posted every day of the week before this shipped and the second had been silent
  since 2026-03-20. Off rather than absent, so it is one tap from coming back.
* The three alert channels — alert, **all**, under their families. On an alert
  row `mentions` means *paged only*: a monitor's post has no `@mention` in it,
  only the subset of its alerts that named the on-call usergroup.
* `#crisp-chats` `C07351C8Z8E` — crisp, **all** (#55).
* Everything else — no row, or a row at **mentions** once listed — which is
  exactly what a channel not on any list did before, minus the hardcoded refusal.

Two properties carried over from #49 rather than lost with it: the `in:#name`
clause on the mention search is still an economy, not the guarantee — `mode: 'off'`
is refused in `bucketHits`, beside the DM rule, whatever Slack does with a query
string — and the test suite pins its own scope, now by asserting the seed rather
than by setting an env var that no longer exists.

**What it costs.** A poll went from four MCP calls (one search, three channel reads)
to twenty-one (one search, twenty channel reads) before thread reads. They run in
one `Promise.allSettled`, so wall time is the slowest read rather than the sum, and
a failed read still costs the whole poll its sweep authority — the property #34
established and this keeps.

**The wholesale rule.** A top-level message in an `all` channel becomes a row when
somebody outside `truto.one`/`truto.dev` posted it (`now` while nobody has answered,
`open` once somebody has) or when a teammate's post already has replies. A
teammate's post with no replies is a broadcast — the weekly changelog every customer
channel receives verbatim — and is skipped. An author with no address at all reads
as external, because a Slack Connect guest often renders without one and the
expensive mistake is skipping a customer's unanswered question. The channel-read
parser also learned that a guest's parenthetical is `(U06…, external: spendflo)`,
which the old `\(([UB][A-Z0-9]+)\)` failed to match, swallowing the whole thing into
the display name.

---

## 55. Crisp is a family, and unresolved is the only state that is on him

There was no Crisp anywhere in the code — not a channel, not a parser. It exists
live: `#crisp-chats`, where the `Truto` bot (`B072X3H2YCF`) posts
`:speech_balloon: Crisp conversation from {visitor}.` per website conversation,
with the back-and-forth as thread replies and one attachment line saying either
`:white_check_mark: Conversation is resolved.` or `:bangbang: Conversation is
unresolved. Please reply.`

It is parsed the way Datadog and Grafana are — structurally, off the attachment
text, keyed on the state word — and lands as `kind: 'crisp'`, title and `who` the
visitor's name, `pile: 'now'` while unresolved and `'open'` once resolved. On the
desk an unresolved one reads `waiting` in the `bad` hue rather than `Chat`, and the
pane carries one banner with the reply count and the Slack link. Resolved ones stay
as history and stay quiet.

Two refusals. An attachment the parser does not recognise reads as **unresolved**:
a visitor genuinely waiting is the expensive miss, and a resolved conversation
sitting in `open` one more poll costs nothing. And the visitor's words are never the
title — the excerpt carries them, redacted and cut like every other Slack card; the
title is the name, which is the one field the desk sorts and searches on.

Human posts in the same channel — Nidhi tagging Uday with a drafted reply — are not
Crisp cards. They follow #54's wholesale rule like any other channel, which is why
`historyCardsForRow` asks who posted before it asks what.

---

## 56. The Sentry tab is the Alerts tab

**Extends the ruling `bucket.ts` already made** — a row belongs to what it is
about, not to the pipe that carried it — one step further than it went. The
first pass parsed a Sentry identity out of a Slack alert and moved those rows
to the Sentry tab, and left Datadog and Grafana pages exactly where they were:
alerts with no Sentry identity, still on the Slack tab, still the first thing a
person reading it for *who is waiting on me* had to scroll past. Measured on the
deployed desk this morning: three of the four rows on the Slack tab were
`API Error Triage in the Last 24 Hours`.

Every production alert is one kind of thing. `bucketOf` files a `sentry` member
and any Slack member with `kind: 'alert'` under `alerts`, whichever monitor wrote
it; the identity-sniffing regex for short ids is gone with it, because the
question stopped being *which monitor* and became *a monitor or a person*, and
`kind` already answers that. `?src=sentry` is still read as `alerts` — a bookmark
or a push link from before must land where it always did — and the strip only
ever writes `alerts`. Settings' *Sources* row still says Sentry, because that is
a connection, and disconnecting one is a different act from tapping a tab.

This is where the Datadog and Grafana signal went when it came off the Slack
tab: nowhere silent. Paged alerts still land in `now`; the per-channel `mode`
(#54) can narrow either monitor to pages only, or switch it off, from Settings.

---

## 57. A card can buzz a phone — for four things, and not a fifth

**Extends #50.** #50 found that no push had ever reached a device because nothing
was subscribed, and fixed the button that had been reporting otherwise. What it
left in place was that even with a device subscribed, no card would ever have
buzzed it: `push.ts` had three callers — a reminder he set, a due date he set, and
the test button — and `ingest()` never called `notify()`. Verified by reading
every call site, not assumed.

`notifyOnNewWork` runs after the ingest transaction commits, over the ids that
transaction wrote, and pushes for exactly:

| kind | when | title | dedup key |
|---|---|---|---|
| 💬 crisp | an unresolved Crisp conversation | `<visitor> is waiting on Crisp` | `crisp:<source_id>:unresolved` |
| 👋 mention | a thread entry that names him and is not his | `<who> mentioned you in #<channel>` | `mention:<group>:<entry ts>` |
| 🚨 paged | an alert naming the on-call usergroup, not a recovery | `Paged: <title>` | `paged:<source_id>` — once, ever, per monitor |
| 🔔 now | a brand-new `now` row with a real person on it, none of the above | `<who> is waiting: <title>` | `now:<group>` |

And it does not push for routine Datadog or Grafana volume, however new. That is
the noise #54 and #56 took off the desk, and a notification would be the same noise
arriving by a louder road; a test pins it.

Two guards run before any of the four. **Nothing older than two hours** — the
first poll after this deploy hands the function a fortnight of newly scoped
customer-channel history as "new" rows, and none of that may become forty buzzes.
**Nothing he has settled** — done, won't-do, not-mine, snoozed. And with nothing
subscribed the function returns before it reads a row, logging once per process.

The push URL is `/?src=<tab>#card/<group_key>`, built from the same two parameters
`route.ts` reads back, and the test round-trips it through `detailKeyOf`. The
service worker used to reduce an absolute target to `.pathname`, which dropped
both — a card's push landed on a bare, unfiltered desk with nothing open.

A fifth kind rides the reminder tick rather than the ingest: **⌨️ session**, when a
live Claude Code session has sat on Claude's own last word, or on a tool call with
no result, for a minute — *and* Wake started it (#39's tmux) *and* no browser is
attached to it. A session he opened in his own terminal never buzzes: he is looking
at it, and Wake telling him about his own screen is the kind of push that gets the
whole feature turned off. The permission-prompt half is a known imprecision — a
`tool_use` with nothing after it looks the same whether it is waiting on a dialog or
still running a long `bash` — and the one-minute floor is what keeps a two-second
`ls` from counting.

---

## 58. Open in Claude stays a session on the box, re-audited

**Confirms #39 after checking the tree as it stands, and records what the prompt
that opened this session asked to be checked.** That prompt was written against a
checkout in which #35's link-only hand-off was still the shipped behaviour and a
later instruction to resume real sessions had "no record of whether it shipped".
It shipped: `terminal.ts`, `ptybridge.py`, `terminalSocket.ts`, `Terminal.tsx`,
and every path the composer's Send takes ends in a tmux-hosted `claude` in the
chosen repository. The operator asked for it twice; the burden is on reverting, and
nothing found here meets it.

The bounds #39 stated hold, with two corrections found on the way:

* argv only, `--` before the brief, mode and model from closed lists, `--resume`
  only for an id `isSessionActive` knows, cwd bounded to `WORKSPACE_ROOT` — all
  as stated, all still tested.
* **The brief's size bound was in the wrong unit.** `MAX_BRIEF_CHARS` measured
  `.length` — UTF-16 code units — against a kernel limit that counts bytes, with a
  comment claiming multi-byte text was safely under it. A CJK ideograph is one
  unit and three bytes, so a support thread quoted in Japanese could pass the
  check and hit the exact E2BIG the check exists to turn into a sentence. It is
  `MAX_BRIEF_BYTES` now, and the refusal names bytes.
* **tmux under a `C` locale silently eats the tab in its list format.** Under
  `LC_ALL=C` — a systemd user unit's default when nothing sets one —
  `-F '#{a}\t#{b}'` prints `a_b`, `infoFrom` splits on a tab that is not there, and
  every running terminal vanishes from `/state` at once with no error on either
  side. `sessionEnv()` substitutes `C.UTF-8` when the inherited locale is missing
  or broken, and a test runs the real spawn-and-list path under `LC_ALL=C`.

Two things the composer said that were no longer true are fixed: the size counter
named a URL limit for a brief that now travels whole as a process argument (the URL
limit applies to the secondary *New chat in the Claude app* link alone, and is said
there, with the trim warning shown on the phone where a `title` cannot be read), and
`templatesFor(card)`'s suggestions were pre-selected but invisible — they are the
first chips now, with a voice template never among them.

`claude remote-control` (in the installed 2.1.x) is Claude's own way to drive a
local session from `claude.ai/code` or the mobile app. It would need a
remote-control daemon the operator starts on the box; Wake starts nothing new for
it and surfaces nothing about it yet. Noted as the path to watch if the browser
terminal ever stops being enough.

---

## 59. The glass refined at the primitive level, and text over it measured

**Extends #52.** The material shipped there; what this session did to it is in
`primitives.tsx`, `motion.ts` and `styles.css` and nowhere page-specific, so every
surface inherits it:

* `Segmented` slides one thumb between options on `spring` via `layoutId`, with
  arrow keys moving the selection, instead of swapping a background per button.
* `Sheet` overshoots slightly on a phone (`softSpring` retuned to a damping ratio
  near 0.7 — a pane thrown by a thumb has momentum) and scales in on a laptop
  (`spring`, fully damped — nothing threw a centred dialog). Its grabber is 36×5 in
  the token the file reserves for grabbers.
* `Menu` scales from the edge nearest its trigger.
* `.glass` panels carry a pointer-tracked radial sheen, written straight to a CSS
  variable on the element under `requestAnimationFrame`, fine pointers only.

**Every one of them takes `useStill()`**, and `test/motion-guard.test.ts` reads
`primitives.tsx` on every run to check that each `initial` falls back to `false`
and each `exit` to `undefined` — the regression that has happened here before,
with no visible symptom until a hidden tab.

**Text over glass is measured, not eyeballed.** `material.test.ts` now composites
`--glass-tint` and `--glass-bar-tint` at their alphas over the page, the ambient
wash, the accent surface `#e9a23b` and both attended row grounds, in both themes,
and holds `fg`, `fg-dim`, `fg-mute`, `accent-ink`, `ok`, `warn` and `bad` at AA on
every one. One number failed and one token moved to pass: `--glass-bar-tint` dark
at 0.66 over amber composited to `#6f5637`, where `warn` measured **3.59:1**, `bad`
3.98, `accent-ink` 4.14, `fg-mute` 4.18. At **0.82** the same four measure 5.01,
